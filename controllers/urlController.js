...
const createShortUrl = async (req,res) =>{
    ...
    if(!original_url || typeof original_url !=='string'){
        return res.status(400).json({ error: 'original_url is required and must be a valid string.'})
    }
    ...
    try{
        ...
        res.json({ short_url:`https://shorturl.devclan.io/${shortUrl}` })
    }catch(error){
...
const redirectToOriginalUrl = async (req,res) =>{
    ...
    try{
        ...
        await sql.query`
            UPDATE url SET click_count = click_count + 1 WHERE short_url = ${short_url}
        `;
